...
func httpWriteHeaderBts(bw *bytes.Buffer, key string, value []byte) {
	httpWriteHeaderKey(bw, key)
	bw.Write(value)
	bw.WriteString(crlf)
}

func httpWriteHeaderKey(bw *bytes.Buffer, key string) {
	bw.WriteString(key)
...
func writeStatusText(bw *bytes.Buffer, code int) {
	bw.WriteString("HTTP/1.1 ")
...
func writeErrorText(bw *bytes.Buffer, err error) {
	body := err.Error()
